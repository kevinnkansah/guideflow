    ...
    const animate = (currentTime: number) => {
      ...
      if (active && currentTime - lastUpdateRef.current > updateRate) {
        ...
        if (analyserRef.current) {
          ...
          if (mode === "static") {
            ...
            // Optimized frequency range for human voice (300Hz-3400Hz)
            const startFreq = Math.floor(dataArray.length * 0.02);
            const endFreq = Math.floor(dataArray.length * 0.15);
            ...
            // Scrolling mode - optimized for human voice
            let sum = 0;
            const startFreq = Math.floor(dataArray.length * 0.02);
            const endFreq = Math.floor(dataArray.length * 0.15);
            const relevantData = dataArray.slice(startFreq, endFreq);
      ...
      if (mode === "static") {
        ...
        for (let i = 0; i < barCount && i < dataToRender.length; i++) {
          ...
          const x = i * step;
          // Improved scaling with logarithmic response for better visualization
          const scaledValue = value ** 0.8; // Soften the response
          const barHeight = Math.max(3, scaledValue * rect.height * 0.85);
          const y = centerY - barHeight / 2;

          ctx.fillStyle = computedBarColor;
          // Enhanced alpha for better visual feedback
          ctx.globalAlpha = 0.3 + scaledValue * 0.7;
        ...
        for (let i = 0; i < barCount && i < historyRef.current.length; i++) {
          ...
          const value = historyRef.current[dataIndex] || 0.05;
          const x = rect.width - (i + 1) * step;
          // Improved scaling for scrolling mode
